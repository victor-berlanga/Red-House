...
(async () => {
  ...
  try {
    ...
    await page.getByLabel('Folio de trazabilidad').fill('RH-BROWSER-001');
    ...
    await page.getByLabel('Grupo sanguíneo registrado').selectOption('A+');
    await page.getByLabel('Referencia del grupo sanguíneo').fill('SOURCE-BROWSER-DEMO');
    await page.getByLabel('Estado inicial').selectOption('AVAILABLE');
    const time = Date.now();
    ...
    await page.waitForURL(root+'/inventario');
    await page.getByText('Unidad registrada correctamente.',{exact:true}).waitFor();
    await page.locator('tbody tr').filter({hasText:'RH-BROWSER-001'}).getByRole('link',{name:/Editar/}).click();
    await page.getByRole('heading',{name:'RH-BROWSER-001',exact:true}).waitFor();
    assert.equal(await page.locator('.inventory-record').count(),1);
    assert.equal(await page.getByLabel('Motivo *',{exact:true}).isVisible(),false);
    await page.getByRole('button',{name:'Guardar cambios',exact:true}).click();
    assert(await page.getByText('No hay cambios de estado o ubicación para guardar.').isVisible());
    await page.getByLabel('Estado',{exact:false}).selectOption('QUARANTINED');
    await page.getByRole('button',{name:'Guardar cambios',exact:true}).click();
    const confirmation=page.locator('#inventory-confirm-dialog');
    assert((await confirmation.innerText()).includes('Disponible → Cuarentena'));
    await confirmation.getByRole('button',{name:'Cancelar',exact:true}).click();
    assert.equal(await page.locator('#current_status').inputValue(),'QUARANTINED');
    await page.getByRole('button',{name:'Guardar cambios',exact:true}).click();
    await confirmation.getByRole('button',{name:'Confirmar cambios',exact:true}).click();
    ...
    await capture('inventory-confirm-save');
    await confirmation.getByRole('button',{name:'Confirmar cambios',exact:true}).click();
    ...
    assert.equal(await page.getByRole('button',{name:'Dar de baja',exact:true}).count(),0);
    await page.goto(root+'/inventario');
    await page.locator('.row-actions a[aria-label^="Editar"]').first().click();
    const withdrawUrl=page.url();
    await page.getByRole('button',{name:'Dar de baja',exact:true}).click();
    assert((await confirmation.innerText()).includes('Los cambios de los desplegables no se guardarán'));
    await page.keyboard.press('Escape');assert.equal(page.url(),withdrawUrl);
    await page.getByRole('button',{name:'Dar de baja',exact:true}).click();
    await confirmation.getByRole('button',{name:'Confirmar baja',exact:true}).click();assert(await confirmation.isVisible());
    await page.getByLabel('Motivo *',{exact:true}).fill('Baja ficticia confirmada desde ventana');
    await page.setViewportSize({width:390,height:844});
    await page.waitForFunction(()=>document.getElementById('sidebar').getBoundingClientRect().right<=1);
    await capture('inventory-confirm-withdraw-mobile');
    await confirmation.getByRole('button',{name:'Confirmar baja',exact:true}).click();
    await page.waitForURL('**/inventario');
    assert(await page.getByText('Unidad dada de baja lógicamente.',{exact:false}).isVisible());
    ...
    await page.waitForFunction(() => new URL(location.href).searchParams.get('q') === 'MOVE' && document.querySelector('[data-filter-results]').getAttribute('aria-busy') === 'false');

...
(async()=>{
 ...
 try {
  ...
  await op.goto(base+'/sangre/personas/donor');await op.locator('[data-record-view]').first().click();await op.locator('#record-dialog [data-detail-content]').waitFor();
  assert.equal(await op.getByRole('heading',{name:'Actualizar expediente',exact:true}).count(),0);
  assert((await op.locator('#record-dialog').innerText()).includes('Expediente de consulta'));
